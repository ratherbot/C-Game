...
namespace Metallolom
{
    public class Controller
    {
        ...
        public static Timer bombsActivatingTimer;
        public static Timer bombsExplodingTimer;
        public static Timer bombsSpawningTimer;
        ...
        private void CheckBombsOut()
        {
            if (model.ExplodedBombs.Count == model.bombsByNumbers.Count)
            {
                model.bombsByNumbers.Clear();
                model.FillMapWithBombs(5);
                view.InitializeBombs();
            }
        }
        ...
        private void SetTimers()
        {
            ...
            enemyMovementTimer.Interval = 120;
            ...
            bombsExplodingTimer = new Timer();
            bombsExplodingTimer.Interval = 3000;
            bombsExplodingTimer.Tick += (sender, e) => ActivateBombs();
            bombsExplodingTimer.Start();

            bombsActivatingTimer = new Timer();
            bombsActivatingTimer.Interval = 400;
            bombsActivatingTimer.Tick += (sender, e) => CheckBombsActivating();
            bombsActivatingTimer.Start();

            bombsSpawningTimer = new Timer();
            bombsSpawningTimer.Interval = 5000;
            bombsSpawningTimer.Tick += (sender, e) => CheckBombsOut();
            bombsSpawningTimer.Start();
        }

        private void CheckBombsActivating()
        {
            
            foreach (var bombNumber in model.bombsByNumbers.Keys)
            {
                model.bombsByNumbers[bombNumber].CheckActivating(model.tanksByNumbers);
                if (model.bombsByNumbers[bombNumber].isActivated)
                    model.ExplodedBombs.Add(bombNumber);
            }
        }
